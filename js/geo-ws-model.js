...
window.com.xomena.geo = {
  ...
  getFormElement: function(id, name, model, triggers, listeners, parentInstance){
    ...
    switch(t){
        case 'string':
            output.push('<input class="pure-input-2-3' + (t_vis?' trigger-visibility':'') + (l_vis?' listen-visibility':'') +
                (t_req?' trigger-required':'') + (l_req?' listen-required':'') + (t_reqOr?' trigger-requiredOr':'')+
                (l_reqOr?' listen-requiredOr':'') + '" type="text" id="parameter-' + id + '" name="' + name + '" value="' +
                (sv.length ? sv[0] : '') + '" size="60"' + (p ?' pattern="' + p + '"' : '') + (h ? ' placeholder="' + h + '"' : '') +
                (r ? ' required' : '') + ' title="' + name + '"/>' +
                '<paper-tooltip for="parameter-'+ id + '" position="right" fit-to-visible-bounds="true" animation-delay="0">' +
                d + '</paper-tooltip>');
            if (m) {
                output.push('<button type="button" name="add-parameter-' + id + '" id="add-parameter-' + id +
                            '" class="add-parameter">Add</button>');
                if(sv.length>1){
                    for(var i=1; i<sv.length; i++){
                        var m_new_id = com.xomena.geo.getNewId();
                        output.push('<br/>');
                        output.push('<input class="pure-input-2-3' + (t_vis ? ' trigger-visibility' : '') +
                            (l_vis ? ' listen-visibility' : '') + (t_req ? ' trigger-required' : '') +
                            (l_req ? ' listen-required' : '') + (t_reqOr ? ' trigger-requiredOr' : '') +
                            (l_reqOr ? ' listen-requiredOr' : '') + '" type="text" id="parameter-' + id + '-' + m_new_id +
                            '" name="' + name + '" value="' + sv[i] + '" size="60"' + (p ? ' pattern="' + p + '"' : '') +
                            (h ? ' placeholder="' + h + '"' : '') + (r ? ' required' : '') + ' title="' + name + '"/>' +
                            '<paper-tooltip for="parameter-'+ id + '-' + m_new_id +
                            '" position="right" fit-to-visible-bounds="true" animation-delay="0">' +
                            d + '</paper-tooltip>');
                    }
                }
            }
            break;
        case 'number':
            output.push('<input class="pure-input-1-2' + (t_vis ? ' trigger-visibility' : '') + (l_vis ? ' listen-visibility' : '') +
                (t_req ? ' trigger-required' : '') + (l_req ? ' listen-required' : '') + (t_reqOr ? ' trigger-requiredOr' : '') +
                (l_reqOr ? ' listen-requiredOr' : '') + '" type="number" id="parameter-' + id + '" name="' + name + '" value="' +
                (sv.length ? sv[0] : '') + '" size="60"' + (p ? ' pattern="' + p + '"' : '') + (h ? ' placeholder="' + h + '"' : '') +
                (r ? ' required' : '') + ' title="' + name + '"/>' +
                '<paper-tooltip for="parameter-' + id + '" position="right" fit-to-visible-bounds="true" animation-delay="0">' +
                d + '</paper-tooltip>');
            if(m){
                ...
                if(sv.length>1){
                    for(var i=1; i<sv.length; i++){
                        var m_new_id = com.xomena.geo.getNewId();
                        output.push('<br/>');
                        output.push('<input class="pure-input-1-2' + (t_vis ? ' trigger-visibility' : '') +
                            (l_vis ? ' listen-visibility' : '') + (t_req ? ' trigger-required' : '') +
                            (l_req ? ' listen-required' : '') + (t_reqOr ? ' trigger-requiredOr' : '') +
                            (l_reqOr ? ' listen-requiredOr' : '') + '" type="number" id="parameter-' + id + '-' + m_new_id +
                            '" name="' + name + '" value="' + sv[i] + '" size="60"' + (p ? ' pattern="' + p + '"' : '') +
                            (h ? ' placeholder="' + h + '"' : '') + (r ? ' required' : '') + ' title="' + name + '"/>' +
                            '<paper-tooltip for="parameter-' + id + '-' + m_new_id +
                            '" position="right" fit-to-visible-bounds="true" animation-delay="0">' + d + '</paper-tooltip>');
                    }
                }
            }
            break;
        case 'list':
            ...
            output.push('<select id="parameter-' + id + '" name="' + name + '" class="pure-input-2-3 chosen-select' +
                (t_vis ? ' trigger-visibility' : '') + (l_vis ? ' listen-visibility' : '') +
                (t_req ? ' trigger-required' : '') + (l_req ? ' listen-required' : '') + (t_reqOr ? ' trigger-requiredOr' : '') +
                (l_reqOr ? ' listen-requiredOr' : '') + '"' + (m ? ' multiple' : '') + (r ? ' required' : '') +
                ' title="' + name + '">' + m_options + '</select>' +
                '<paper-tooltip for="parameter-' + id +
                '" position="right" fit-to-visible-bounds="true" animation-delay="0">' +
                d + '</paper-tooltip>');
            break;
        case 'checkboxes':
            ...
            _.each(ao, function(opt, ind){
                ...
                var l = a1.length>1?a1[1]:a1[0];
                output.push('<li>');
                output.push('<label for="parameter-' + id + '-' + ind + '" title="' + name +
                ...
                output.push(l);
                output.push('</label>');
                output.push('<paper-tooltip for="label-parameter-' + id + '-' + ind +
                            '" position="right" fit-to-visible-bounds="true" animation-delay="0">' + d +'</paper-tooltip>');
                output.push('</li>');
            });
            output.push('</ul>');
        ...
        case 'checkbox':
            ...
            var v = a1[0];
            var l = a1.length>1?a1[1]:a1[0];
            output.push('<label for="parameter-' + id + '" title="' + name +
            ...
            output.push(l);
            output.push('</label>');
            output.push('<paper-tooltip for="label-parameter-' + id +
                        '" position="right" fit-to-visible-bounds="true" animation-delay="0">' +
                        d +'</paper-tooltip>');
            break;
        case 'parts':
        ...
        case 'timestamp':
            output.push('<input class="pure-input-2-3' + (t_vis ? ' trigger-visibility' : '') +
                (l_vis ? ' listen-visibility' : '') + (t_req ? ' trigger-required' : '') + (l_req ? ' listen-required' : '') +
                (t_reqOr ? ' trigger-requiredOr' : '') + (l_reqOr ? ' listen-requiredOr' : '') +
                '" type="text" id="parameter-' + id + '" name="' + name + '" value="' + (sv.length ? sv[0] : '') +
                '" size="60"' + (p ? ' pattern="' + p + '"' : '') + (h ? ' placeholder="' + h + '"' : '') +
                (r ? ' required' : '') + 'title="' + name + '"/>' +
                '<paper-tooltip for="parameter-' + id + '" position="right" fit-to-visible-bounds="true" animation-delay="0">' +
                d +'</paper-tooltip>');
            break;
  ...
  chooseWebService: function(ev){
      ...
      if(ev.target.value){
        ...
        this.$(".chosen-select").each(function () {
            ...
            var m_rep_id = m_id.replace(/\-/g, "_") + "_chosen";
            var m_text = $("paper-tooltip[for='" + m_id + "']").text();
            $('<paper-tooltip for="'+ m_rep_id + '" position="right" fit-to-visible-bounds="true" animation-delay="0">' +
                m_text + '</paper-tooltip>').insertAfter("#"+m_rep_id);
        });
