...
window.com.xomena.geo = {
    ...
    switch(t){
        case 'string':
            output.push('<input class="pure-input-2-3' + (t_vis?' trigger-visibility':'') + (l_vis?' listen-visibility':'') +
                ...
                (r ? ' required' : '') + ' title="' + name + '"/>' + com.xomena.geo.getTooltipElement('parameter-'+ id, d));
            if (m) {
                ...
                if(sv.length>1){
                    for(var i=1; i<sv.length; i++){
                        ...
                        output.push('<input class="pure-input-2-3' + (t_vis ? ' trigger-visibility' : '') +
                            ...
                            com.xomena.geo.getTooltipElement('parameter-'+ id + '-' + m_new_id, d));
                    }
                }
            }
            break;
        case 'number':
            output.push('<input class="pure-input-1-2' + (t_vis ? ' trigger-visibility' : '') + (l_vis ? ' listen-visibility' : '') +
                ...
                com.xomena.geo.getTooltipElement('parameter-'+ id, d));
            if(m){
                output.push('<button type="button" name="add-parameter-' + id + '" id="add-parameter-' + id +
                            '" class="add-parameter">Add</button>');
                if(sv.length>1){
        ...
        case 'checkboxes':
            ...
            _.each(ao, function(opt, ind){
                ...
                output.push(l);
                output.push(com.xomena.geo.getTooltipElement('parameter-' + id + '-' + ind, d));
                output.push('</label>');
                output.push('</li>');
            });
            output.push('</ul>');
        ...
        case 'checkbox':
            ...
            output.push('<input type="checkbox" id="parameter-' + id + '" name="' + name + '" value="' + v + '"' +
                ...
                (t_reqOr ? ' trigger-requiredOr' : '') + (l_reqOr ? ' listen-requiredOr' : '') + '"/>');
            output.push(l);
            output.push(com.xomena.geo.getTooltipElement('parameter-' + id, d));
            output.push('</label>');
            break;
        case 'parts':
  ...
  getTooltipElement: function (elemId, text) {
    var res = ['<div class="card-tooltip">'];
